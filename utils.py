...
def auto_load_model(args, model, model_without_ddp, optimizer, loss_scaler, model_ema=None):
    ...
    if args.resume:
        ...
        try:
            ...
            for k1, k2 in zip(state_dict, ckp_dict):
                print(k1, state_dict[k1].shape, k2, ckp_dict[k2].shape)
                if (state_dict[k1].shape == ckp_dict[k2].shape):
                    # copy directly
                    new_dict[k1] = ckp_dict[k2]
                else:
                    try:
                        new_dict[k1] = reloc_w(ckp_dict[k2])
                    except Exception as e:
                        print('@@@@@@@@@@@@@@@@@@@',k1,k2)
                        raise(e)
...
def reloc_w(w):
    ...
    kk1 = min(k1, k2)
    kk2 = max(k1, k2)
    nk = math.ceil(kk2 / kk1)

    if kk1 == k1:
        pad_r = kk1 - w.shape[3] % kk1
        w = F.pad(w, (0, pad_r, 0, 0))
        # w = torch.split(w, k1, dim=3)
        # w = torch.cat(w, dim=0)
        w = torch.split(w.unsqueeze(1), kk1, dim=3 + 1)  # ※※※※※※※※
        w = torch.cat(w, dim=1).reshape(-1, c2, kk1, kk1)  # ※※※※※※※※
    else:
        pad_r = kk1 - w.shape[2] % kk1
        w = F.pad(w, (0, 0, 0, pad_r))
        # w = torch.split(w, k1, dim=2)
        # w = torch.cat(w, dim=0)
        w = torch.split(w.unsqueeze(1), kk1, dim=2 + 1)
        w = torch.cat(w, dim=1).reshape(-1, c2, kk1, kk1)
    return w
