...
from .dw_gemm import get_conv2d_general as get_conv2d
use_sync_bn = True
...
class Block(nn.Module):
    ...
    """
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6, kernel_size=(7,7), Decom=None, bn=True, small_kernel_merged=False):
        super().__init__()
        ...
        self.large_kernel = ReparamLargeKernelConv(in_channels=dim, out_channels=dim,
                                                   ...
                                                   small_kernel_merged=small_kernel_merged, Decom=Decom, bn=bn)
...
class SLaK(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000, 
                 ...
                 small_kernel_merged=False, **kwargs
                 ):
        super().__init__()
        ...
        for i in range(4):
            stage = nn.Sequential(
                *[Block(dim=dims[i], drop_path=dp_rates[cur + j], 
                layer_scale_init_value=layer_scale_init_value, kernel_size=(self.kernel_size[i], self.kernel_size[-1]), 
                Decom=Decom, bn=bn, small_kernel_merged=small_kernel_merged) for j in range(depths[i])]
            )
