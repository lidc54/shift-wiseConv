...
def conv_bn(in_channels, out_channels, kernel_size, stride, padding, groups, dilation=1, bn=True, use_small_conv=True):
    if isinstance(kernel_size, int) or len(set(kernel_size)) == 1:
        ...
        # torch.jit.script(
        return LoRAConvsByRandom(in_channels, out_channels, bn=bn,
...
class Block(nn.Module):
    r"""SLaK_gr_swsf_se Block. There are two equivalent implementations:
...
# patch-wise
class SLaK_gr_swsf_se(nn.Module):
    r"""SLaK_gr_swsf_se
...
# fp:fixing padding
@register_model
def SLaK_gr_swsf_se_tiny(pretrained=False, **kwargs):
    model = SLaK_gr_swsf_se(depths=[3, 3, 9, 3], dims=[96, 192, 384, 768], **kwargs)

    return model


@register_model
def SLaK_gr_swsf_se_small(pretrained=False, **kwargs):
    model = SLaK_gr_swsf_se(depths=[3, 3, 27, 3], dims=[96, 192, 384, 768], **kwargs)

    return model


@register_model
def SLaK_gr_swsf_se_base(pretrained=False, in_22k=False, **kwargs):
    model = SLaK_gr_swsf_se(depths=[3, 3, 27, 3], dims=[128, 256, 512, 1024], **kwargs)

    return model


@register_model
def SLaK_gr_swsf_se_large(pretrained=False, in_22k=False, **kwargs):
    model = SLaK_gr_swsf_se(depths=[3, 3, 27, 3], dims=[192, 384, 768, 1536], **kwargs)
