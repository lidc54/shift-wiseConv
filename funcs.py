...
def magnitude_prune_sum(masking, mask, weight, name):
    """
    prune a conv  in coarse grain
    """
    num_remove = math.ceil(masking.prune_rate*masking.name2nonzeros[name])
    num_zeros = masking.name2zeros[name]
    k = math.ceil(num_zeros + num_remove)
    ...
    if num_remove == 0.0: return w != 0.0
...
def random_growth_sum(masking, name, new_mask, total_regrowth, weight):
    n = (new_mask==0).sum().item()
    if n == 0: return new_mask
    expeced_growth_probability = (total_regrowth/n)
    new_weights = torch.rand(new_mask.shape).cuda() < expeced_growth_probability
...
prune_funcs = {}
prune_funcs['magnitude_sum'] = magnitude_prune_sum
...
growth_funcs = {}
growth_funcs['random_sum'] = random_growth_sum
